...
# from eff_word_net.audio_processing import Resnet50_Arc_loss  # Commented: wake-word disabled
...
# 
# Wake-word detector disabled - return None instead
def load_wake_word_detector() -> None:
    """Wake-word detection is disabled."""
    logger.info("Wake-word detection is disabled (commented out)")
    return None


def load_all_models() -> Tuple[torch.nn.Module, DF, int, str, None]:
    """Load all models (DeepFilterNet only, wake-word disabled)."""
    logger.info("Loading models (wake-word disabled)...")
    model, df_state, target_sr, device = load_deepfilternet()
    wake_word_detector = load_wake_word_detector()  # Returns None (wake-word disabled)
    logger.info("Models loaded successfully (wake-word disabled)")
